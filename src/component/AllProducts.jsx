    ...
    if (loading) {
        return (
            <div className='min-h-screen bg-neutral-950 py-20 px-6 flex items-center justify-center'>
                <div className="text-neutral-400 text-lg">Loading all products...</div>
            </div>
        );
    }
                        ...
                        >
                            ...
                            {hasActiveFilters && (
                                <span className="px-2 py-0.5 bg-cyan-500 text-white text-xs rounded-full font-semibold">
                                    {selectedCategories.length + selectedBrands.length + selectedRatings.length + (priceRange.min || priceRange.max ? 1 : 0)}
                                </span>
                            )}
                        </button>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {filteredProducts.map((product) => (
                            <div
                                ...
                                onClick={() => navigate(`/tech/${product.category}/${product.productId}`)}
                                className="group relative bg-neutral-900/30 border border-white/5 overflow-hidden cursor-pointer hover:border-white/10 transition-all duration-300 hover:shadow-xl"
                            >
                                {/* Product Image */}
                                    ...
                                    {product.imageUrl ? (
                                        <img
                                            ...
                                            onError={(e) => {
                                                ...
                                                e.target.parentElement.innerHTML = `<div class="text-6xl opacity-20">${categories.find(c => c.value === product.category)?.icon || '📦'}</div>`;
                                            }}
                                        />
                                    ) : (
                                ...
                                <div className="p-5">
                                    ...
                                    <div className="flex items-center gap-2 mb-4 pb-4 border-b border-white/5">
                                        <div className="flex gap-0.5">
                                            {[...Array(5)].map((_, i) => (
                                                <svg
                                                    key={i}
                                                    className={`w-3 h-3 ${i < Math.floor(product.rating || 0) ? 'text-yellow-400' : 'text-neutral-700'}`}
                                                    fill="currentColor"
                                                    viewBox="0 0 20 20"
                                                >
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className="text-xs text-neutral-500 group-hover:text-cyan-400 transition-colors">
                                            View →
                                        </span>
                                    </div>
                                </div>
